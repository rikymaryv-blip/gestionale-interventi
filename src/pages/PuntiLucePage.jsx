...
export default function PuntiLucePage() {
  ...
  const capitoli = [...new Set(vociDB.map(v => v.capitolo).filter(Boolean))]

  const vociCapitolo = vociDB.filter(v => v.capitolo === nuovo.capitolo)

  function getVoceConfig() {
    return vociDB.find(v =>
      v.capitolo === nuovo.capitolo &&
      v.voce === nuovo.tipo
    )
  }
  ...
  function postiEffettivi() {
    ...
    if (voce.richiede_posti) {
      return Number(nuovo.posti || voce.posti_default || 1)
    }
  ...
  function descrizioneAutomatica() {
    ...
    if (voce.richiede_posti) {
      return `${voce.voce} da ${postiEffettivi()} posti`
    }
  ...
  function calcolaModuliPunto(punto) {
    const config = vociDB.find(v =>
      v.capitolo === punto.capitolo &&
      v.voce === punto.tipo
    )
  ...
  const moduliTotaliCorrente = calcolaModuliDaScatole(stanzaCorrente.scatole)
  const moduliUsatiCorrente = calcolaModuliUsati(stanzaCorrente.punti)
  ...
  function aggiungiPunto() {
    ...
    const moduliNuovo = calcolaModuliPunto(nuovoPunto)
  ...
  function salvaStanzaCorrente() {
    ...
    const esiste = stanze.find(s => s.id === stanzaCorrente.id)

    if (esiste) {
      setStanze(stanze.map(s =>
        s.id === stanzaCorrente.id ? stanzaCorrente : s
      ))
    } else {
      setStanze([...stanze, stanzaCorrente])
    }
  }
  ...
  function eliminaPunto(id) {
    setStanzaCorrente({
      ...stanzaCorrente,
      punti: stanzaCorrente.punti.filter(p => p.id !== id)
    })
  }
  ...
  const stanzeDaMostrare = [
    ...stanze.filter(s => s.id !== stanzaCorrente.id),
    ...(stanzaCorrente.nome.trim() ? [stanzaCorrente] : [])
  ]
  ...
  return (
    <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => cambiaNomeStanza(e.target.value)}
          ...
          <button onClick={nuovaStanza} style={btnVerde}>
            ➕ Stanza
          </button>
        </div>
          ...
          {scatoleDisponibili.map(s => (
            <div key={s.codice}>
              ...
              <input
                type="number"
                min="0"
                value={stanzaCorrente.scatole[s.codice]}
                onChange={(e) => aggiornaScatola(s.codice, e.target.value)}
                style={input}
              />
            </div>
          ))}
      ...
      <div style={{
        ...
        <div style={{
          display: "grid",
          gridTemplateColumns: "90px 170px 240px 120px 1fr 120px",
          gap: 8,
          alignItems: "end"
        }}>
          <div>
          ...
          <div>
            ...
            <input
              type="number"
              min="1"
              disabled={!getVoceConfig()?.richiede_posti}
              value={postiEffettivi()}
              onChange={(e) => setNuovo({ ...nuovo, posti: e.target.value })}
              style={{
                ...input,
                background: !getVoceConfig()?.richiede_posti ? "#eee" : "white"
              }}
            />
          </div>
          ...
          <button onClick={aggiungiPunto} style={btnVerde}>
            Aggiungi
          </button>
        </div>
      ...
      <div style={box}>
        ...
        <div style={{ marginBottom: 10 }}>
          <b>Cliente:</b> {clienteSelezionato?.nome || "-"} {" | "}
          <b>Serie:</b> {serie || "-"}
        </div>
        ...
        {stanzeDaMostrare.map(stanza => {
          const moduliTotali = calcolaModuliDaScatole(stanza.scatole)
          const moduliUsati = calcolaModuliUsati(stanza.punti)
          const moduliRimasti = moduliTotali - moduliUsati
          ...
          return (
            <div
              ...
              style={{
                marginTop: 20,
                padding: 10,
                border: stanza.id === stanzaCorrente.id ? "3px solid #0d6efd" : "2px solid #ddd",
                borderRadius: 8,
                background: "white"
              }}
            >
              ...
              <div style={{
                ...
                <MiniBox titolo="Totali" valore={moduliTotali} />
                <MiniBox titolo="Usati" valore={moduliUsati} />
                <MiniBox titolo="Rimasti" valore={moduliRimasti} danger={moduliRimasti < 0} />
              </div>
              ...
              <table style={{
                ...
                <tbody>
                  {scatoleDisponibili.map(s => {
                    const qta = stanza.scatole[s.codice]
                    if (!qta) return null
                    ...
                    return (
                      <tr key={s.codice}>
                        <td style={td}>{qta}</td>
                        <td style={td}>
                          {s.descrizioneSupporto} + {s.descrizionePlacca}
                        </td>
                      </tr>
                    )
                  })}
              ...
              {capitoli.map(capitolo => {
                const lista = stanza.punti.filter(p => p.capitolo === capitolo)
                    ...
                    <table style={{
                      ...
                      <tbody>
                        {lista.map(p => (
                          <tr
                            key={p.id}
                            ref={p.id === ultimoId ? ultimoRef : null}
                            style={{
                              background: p.id === ultimoId ? "#fff3cd" : "white"
                            }}
                          >
...
function MiniBox({ titolo, valore, danger = false }) {
  return (
    <div style={{
      ...
      borderRadius: 6,
      padding: 6,
      background: danger ? "#f8d7da" : "#f8f9fa",
      textAlign: "center"
    }}>
      <b>{titolo}</b>
